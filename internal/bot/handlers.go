...
func (h *MessageHandler) handleMenuCommand(update *tgbotapi.Update, bot *tgbotapi.BotAPI, text string) error {
	...
	// Check for "My Profile" buttons first
	if strings.HasPrefix(text, MyProfilePrefix) {
		return h.messageGen.HandleMyProfileRequest(update, bot)
	}
...
func (h *CallbackHandler) HandleCallback(update *tgbotapi.Update, bot *tgbotapi.BotAPI) error {
	...
	case SubscriptionPeriodCallback:
		if len(parts) < 2 {
			return fmt.Errorf("invalid subscription period callback")
		}
		subscriptionKey := parts[1] // format: premium_7, pro_plus_30, etc.

		return h.messageGen.HandlePaymentRequest(update, bot, subscriptionKey)
